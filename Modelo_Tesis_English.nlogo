...
ask trenes[
    ...
    set newnode one-of [out-link-neighbors] of mynode
      ifelse (newnode = nobody) [renueva] [face newnode ifelse (prob_cc) <= short_circuit_probability and (prob_cc) != 0 [
        ...
        (ifelse (prob_heridos) <= injured_users_probability and (prob_heridos) != 0 [set heridos? true crearDestin llamar_ambulancia if (prob_incendio) <= fire_hazard_probability and (prob_incendio) != 0 [set incendio? true crearDestin llamar_bomberos]]
          (prob_incendio) <= fire_hazard_probability and (prob_incendio) != 0 [set incendio? true set heridos? false crearDestin llamar_bomberos][clear-output output-type "El tren ha sido reparado." set color previouscolor set descompuesto? false
   set prob_CC random-float 100 if prob_CC = 0 [ set prob_CC random-float 100]
      set prob_heridos random-float 100 if prob_heridos = 0 [ set prob_heridos random-float 100]
...
if show_Main_Avenues? = false   [user-message "Está desactivada la visualización de avenidas principales, no se puede calcular la ruta más corta" stop]
...
if use_Dijkstra? = true [create-shortest-path] wait 2
let distancia_ruta []
let ruta []
...
print (word "The shortest route bewteen the closest hospital and the incident on the line " linea " is: " ruta " for a total distance of: " sum distancia_ruta)
...
to llamar_bomberos
 if show_Fire_Stations? = false [user-message "Está desactivada la visualización de Estaciones de Bomberos" stop]
...
let ruta []
ask turtle Origin [foreach (nw:path-to turtle Destin) [x -> ask x [ set distancia_ruta lput(link-length)  distancia_ruta]] set ruta nw:path-to turtle Destin]
print (word "The shortest route between the closest firestation and the incident on the line " linea " is: " ruta " for a total distance of: " sum distancia_ruta)
...
to create-shortest-path
  ...
  if heridos? = true [ print (word "The shortest route between the closest hospital and the incident on the line " linea " is: " final_route " for a total distance of: " total_distance)]
  if heridos? = false [print (word "The shortest route between the closest firestation and the incident on the line " linea " is: " final_route " for a total distance of: " total_distance)]
  let i 0
...
1548
367
Incidents involving Ambulances
IncidentesAmbulancias
17
1
11
...
1559
419
Incidentes involving Fire Fighters
IncidentesBomberos
17
1
11
...
1695
781
Mean distance FireFighters L12
distancia_media_bom_L12
17
1
11
